...
export default function parseHTMLRunCode(source) {
    ...
    return {
        code,
        beforeHTML:_.get(new RegExp(/([\s\S]+)<!--RunCode-->\s*(```jsx?)([\s\S]+)(```)\s*<!--\/RunCode-->/gi).exec(source),'1',''),
        afterHTML:_.get(new RegExp(/<!--RunCode-->\s*(```jsx?)([\s\S]+)(```)\s*<!--\/RunCode-->([\s\S]+)/gi).exec(source),'4','')
    };
}
